  ...
  0x09: { // DAD B
    ...
    execute: (cpu, bytes) => {
      ...
      switch(cpu.currentCycle) {
        ...
        case 5:
          cpu.setBufReg2(cpu.registers.C);
          break;
              
        case 6:
          // Загрузка младшего байта HL в буферный регистр 1
          let lowByte;
          if (rp === 0) {
            lowByte = cpu.registers.C;      // DAD B
          }
          else if (rp === 1) {
            lowByte = cpu.registers.E;      // DAD D
          }
          else if (rp === 2) {
            lowByte = cpu.registers.L;      // DAD H
          }
          else if (rp === 3) {
            lowByte = (cpu.registers.SP >> 8) & 0xFF; // DAD SP
          }
          cpu.setBufReg2(lowByte);
          break;
              
        case 7:
          // Сложение младших байтов
          const sumLow = cpu.registers.L + cpu.bufReg2;
          cpu.registers.L = sumLow & 0xFF;
          cpu.flags.C = sumLow > 0xFF ? 1 : 0;
          break;
              
        case 8:
          // Загрузка старшего байта регистровой пары в буферный регистр 2
          let highByte;
          if (rp === 0) {
            highByte = cpu.registers.B;      // DAD B
          }
          else if (rp === 1) {
            highByte = cpu.registers.D;      // DAD D
          }
          else if (rp === 2) {
            highByte = cpu.registers.H;      // DAD H
          }
          else if (rp === 3) {
            highByte = (cpu.registers.SP >> 8) & 0xFF; // DAD SP
          }
          cpu.setBufReg2(highByte);
          break;
              
        case 9:
          // Загрузка старшего байта HL в буферный регистр 1
          cpu.setBufReg1(cpu.registers.H);
          break;
              
        case 10:
          // Сложение старших байтов с учетом переноса
          const sumHigh = cpu.registers.H + cpu.bufReg2 + (cpu.flags.C ? 1 : 0);
  ...
  0x29: { // DAD H
    ...
    execute: (cpu, bytes) => {
      switch(cpu.currentCycle) {
        ...
        case 7:
          const sumLow = cpu.registers.L * 2;
          cpu.registers.L = sumLow & 0xFF;
          cpu.flags.C = sumLow > 0xFF ? 1 : 0;
          break;
        ...
        case 10:
          const sumHigh = cpu.registers.H * 2 + (cpu.flags.C ? 1 : 0);
          cpu.registers.H = sumHigh & 0xFF;
          cpu.flags.C = sumHigh > 0xFF ? 1 : cpu.flags.C;
          break;
      }
  ...
  0x39: { // DAD SP
    ...
    execute: (cpu, bytes) => {
      switch(cpu.currentCycle) {
        ...
        case 5:
          // Загрузка младшего байта SP в буферный регистр 2
          cpu.setBufReg2(cpu.registers.SP & 0xFF);
          break;
        ...
        case 8:
          // Загрузка старшего байта SP в буферный регистр 2
          cpu.setBufReg2((cpu.registers.SP >> 8) & 0xFF);
          break;
